...
// List all packs
router.get('/', requirePermission('ads_manage'), (req, res) => {
  res.json(adPackService.getAllPacks());
});

// Get single pack
router.get('/:id', requirePermission('ads_manage'), (req, res) => {
...
// Create pack
router.post('/', requirePermission('ads_manage'), (req, res) => {
  const { name } = req.body;
...
// Update pack name
router.put('/:id', requirePermission('ads_manage'), (req, res) => {
  const { name } = req.body;
  if (!name) return res.status(400).json({ error: 'Name required' });
...
// Delete pack
router.delete('/:id', requirePermission('ads_manage'), (req, res) => {
  adPackService.deletePack(parseInt(req.params.id));
  res.json({ ok: true });
});

// Add ad to pack
router.post('/:id/items', requirePermission('ads_manage'), (req, res) => {
...
// Remove ad from pack
router.delete('/:id/items/:adId', requirePermission('ads_manage'), (req, res) => {
  res.json(adPackService.removeItem(parseInt(req.params.id), parseInt(req.params.adId)));
});

// Add assignment
router.post('/:id/assign', requirePermission('ads_manage'), (req, res) => {
...
// Remove assignment
router.delete('/assignments/:assignId', requirePermission('ads_manage'), (req, res) => {
